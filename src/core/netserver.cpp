...
static void makePresetKey(char *out, size_t outSz, uint8_t bank, uint8_t slot, const char *suffix) {
  if (bank == 0) {
...
static bool decodeUtf8CodePoint(const uint8_t *data, size_t len, size_t idx, uint32_t &cp, size_t &seqLen) {
  if (idx >= len) {
    return false;
  }
  ...
  if ((b0 & 0xF8) == 0xF0) {
    ...
    const uint8_t b1 = data[idx + 1];
    const uint8_t b2 = data[idx + 2];
    const uint8_t b3 = data[idx + 3];
    if ((b1 & 0xC0) != 0x80 || (b2 & 0xC0) != 0x80 || (b3 & 0xC0) != 0x80) {
      return false;
    }
    cp = ((uint32_t)(b0 & 0x07) << 18) | ((uint32_t)(b1 & 0x3F) << 12) | ((uint32_t)(b2 & 0x3F) << 6) | (uint32_t)(b3 & 0x3F);
...
static String defaultFavLabel(uint8_t fav) {
  return String("FAV") + String(static_cast<unsigned>(fav + 1));
}

static String sanitizeFavLabel(const String &input, uint8_t fav, bool *usedFallback = nullptr) {
  const uint8_t *bytes = reinterpret_cast<const uint8_t *>(input.c_str());
  const size_t len = static_cast<size_t>(input.length());
  String out;
  out.reserve(len);
  ...
  for (size_t i = 0; i < len;) {
    uint32_t cp = 0;
    size_t seqLen = 0;
    if (!decodeUtf8CodePoint(bytes, len, i, cp, seqLen)) {
      invalidFound = true;
      i++;
      continue;
    }
    ...
    for (size_t k = 0; k < seqLen; k++) {
      out += static_cast<char>(bytes[i + k]);
    }
    i += seqLen;
  }
  ...
  const bool fallback = invalidFound || out.length() == 0;
  if (usedFallback) {
    *usedFallback = fallback;
  }
  if (fallback) {
    return defaultFavLabel(fav);
  }
  if (out.length() > 64) {
...
static int splitTabLine(const String &line, String tokens[], int maxTokens) {
  int tokenCount = 0;
  int start = 0;
  for (int i = 0; i <= static_cast<int>(line.length()) && tokenCount < maxTokens; i++) {
    bool isDelimiter = (i == static_cast<int>(line.length())) || line[i] == '\t';
    if (!isDelimiter) {
      continue;
    }
...
static String buildPresetsCsv() {
  static constexpr uint8_t PRESET_BANKS = 5;
  static constexpr uint8_t PRESET_SLOTS = 8;

  Preferences prefs;
  if (!prefs.begin("presets", false)) {
    return "selected_bank\t0\n";
  }
  ...
  out += "selected_bank\t";
  out += String(static_cast<unsigned>(prefs.getUChar("fav_sel", 0)));
  out += "\n";
  ...
  for (uint8_t bank = 0; bank < PRESET_BANKS; bank++) {
    snprintf(key, sizeof(key), "fav%u_label", bank);
    prefs.getString(key, label, sizeof(label));
    bool labelFallback = false;
    String safeLabel = sanitizeFavLabel(String(label), bank, &labelFallback);
    if (labelFallback) {
      prefs.putString(key, safeLabel);
    }
    safeLabel.replace("\t", " ");
    safeLabel.replace("\n", " ");
    safeLabel.replace("\r", " ");
    ...
    for (uint8_t slot = 0; slot < PRESET_SLOTS; slot++) {
      ...
      makePresetKey(key, sizeof(key), bank, slot, "name");
      prefs.getString(key, name, sizeof(name));
      makePresetKey(key, sizeof(key), bank, slot, "url");
      prefs.getString(key, url, sizeof(url));
      makePresetKey(key, sizeof(key), bank, slot, "id");
      uint16_t id = prefs.getUShort(key, 0);
...
static bool importPresetsCsv(const String &csv, String &error) {
  static constexpr uint8_t PRESET_BANKS = 5;
  static constexpr uint8_t PRESET_SLOTS = 8;

  Preferences prefs;
  if (!prefs.begin("presets", false)) {
    error = "cannot open presets storage";
    return false;
  }
  ...
  while (pos <= static_cast<int>(csv.length())) {
    ...
    String cols[6];
    int colCount = splitTabLine(line, cols, 6);
    if (colCount < 2) {
      continue;
    }

    if (cols[0] == "selected_bank") {
      char *end = nullptr;
      long bank = strtol(cols[1].c_str(), &end, 10);
      if (end == cols[1].c_str() || *end != '\0' || bank < 0 || bank >= PRESET_BANKS) {
        prefs.end();
        error = "selected_bank out of range";
        return false;
      }
      selectedBank = static_cast<uint8_t>(bank);
    ...
    if (cols[0] == "fav_label") {
      ...
      if (end == cols[1].c_str() || *end != '\0' || bank < 0 || bank >= PRESET_BANKS) {
        prefs.end();
        error = "fav_label bank out of range";
        return false;
      }
      String label = sanitizeFavLabel(cols[2], static_cast<uint8_t>(bank));
      snprintf(key, sizeof(key), "fav%ld_label", bank);
    ...
    if (cols[0] == "preset") {
      if (colCount < 6) {
        prefs.end();
        error = "invalid preset row";
        return false;
      }

      char *bankEnd = nullptr;
      long bank = strtol(cols[1].c_str(), &bankEnd, 10);
      char *slotEnd = nullptr;
      long slot = strtol(cols[2].c_str(), &slotEnd, 10);
      char *idEnd = nullptr;
      long id = strtol(cols[5].c_str(), &idEnd, 10);

      if (bankEnd == cols[1].c_str() || *bankEnd != '\0' || bank < 0 || bank >= PRESET_BANKS) {
        prefs.end();
        error = "preset bank out of range";
        return false;
      }
      if (slotEnd == cols[2].c_str() || *slotEnd != '\0' || slot < 0 || slot >= PRESET_SLOTS) {
        prefs.end();
        error = "preset slot out of range";
        return false;
      }
      if (idEnd == cols[5].c_str() || *idEnd != '\0' || id < 0 || id > 65535) {
        prefs.end();
        error = "preset id out of range";
        return false;
      }

      const String &name = cols[3];
      const String &url = cols[4];

      if (name.length()) {
        makePresetKey(key, sizeof(key), static_cast<uint8_t>(bank), static_cast<uint8_t>(slot), "name");
        prefs.putString(key, name);
      }
      if (url.length()) {
        makePresetKey(key, sizeof(key), static_cast<uint8_t>(bank), static_cast<uint8_t>(slot), "url");
        prefs.putString(key, url);
      }
      if (id > 0) {
        makePresetKey(key, sizeof(key), static_cast<uint8_t>(bank), static_cast<uint8_t>(slot), "id");
        prefs.putUShort(key, static_cast<uint16_t>(id));
      }
      importedAnything = true;
    }
  }
  ...
  if (!importedAnything) {
    prefs.end();
    error = "no preset rows found";
    return false;
  }

  if (hasSelectedBank) {
    prefs.putUChar("fav_sel", selectedBank);
  }
...
static String buildIrCodesCsv() {
  ...
  char valBuf[32];
  for (int btn = 0; btn < IR_BUTTON_COUNT; btn++) {
    out += String(btn);
...
static bool importIrCodesCsv(const String &csv, String &error) {
  ...
  while (pos <= static_cast<int>(csv.length())) {
    ...
    if (btnIdx < 0 || btnIdx >= IR_BUTTON_COUNT) {
...
//DLNA mod
  ...
  webserver.on("/presets.csv", HTTP_GET, [](AsyncWebServerRequest *request) {
    String csv = buildPresetsCsv();

    if (!LittleFS.exists("/data")) {
      LittleFS.mkdir("/data");
    }
    File exportFile = LittleFS.open("/data/presets.csv", "w");
    if (exportFile) {
      exportFile.print(csv);
      exportFile.close();
    }
    ...
    response->addHeader("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0");
    response->addHeader("Pragma", "no-cache");
    response->addHeader("Content-Disposition", "attachment; filename=\"presets.csv\"");
    request->send(response);
  });

  webserver.on(
    "/presets.csv", HTTP_POST,
    [](AsyncWebServerRequest *request) {
      if (!request->_tempFile) {
        request->send(400, "application/json", "{\"error\":\"csv body missing\"}");
        return;
      ...
      String importError;
      if (!importPresetsCsv(csv, importError)) {
        importError.replace("\\", "\\\\");
        importError.replace("\"", "\\\"");
        request->send(400, "application/json", "{\"error\":\"" + importError + "\"}");
        return;
...
#ifndef NETSERVER_LOOP1
  display.lock();
  // If the client aborts mid-transfer the chunked callback never reaches EOF, so unlock here too.

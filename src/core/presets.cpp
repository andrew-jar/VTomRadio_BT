...
static void presetsBlitRect(int16_t x, int16_t y, int16_t w, int16_t h) {
    ...
    // Clip right/bottom bounds.
    int16_t maxW = _spr->width() - x;
    int16_t maxH = _spr->height() - y;
    if (w > maxW) { w = maxW; }
    if (h > maxH) { h = maxH; }

    if (w <= 0 || h <= 0) { return; }

    const uint16_t* fb = static_cast<const uint16_t*>(_spr->getBuffer());
    if (!fb) { return; }

    const int16_t sprW = _spr->width();
    dsp.startWrite();
    for (int16_t row = 0; row < h; ++row) {
        const uint16_t* line = fb + (y + row) * sprW + x;
        dsp.pushImage(x, y + row, w, 1, line);
    }
    dsp.endWrite();
}
...
static bool          s_sanitizedBanks[5] = {false, false, false, false, false}; // sanitize only once per bank
...
static void loadBankCache() {
    ensurePrefs();
    if (s_bank < 5 && !s_sanitizedBanks[s_bank]) {
        sanitizePresets(s_bank);
        s_sanitizedBanks[s_bank] = true;
    }
...
void presets_drawPressed() {
    if (s_pressedSlot < 0 || s_pressedSlot >= SLOTS) { return; }
    if (s_pressedSlot == s_lastDrawnPressed) { return; }
    // Rendering is handled by presets_drawScreen() from display thread.
...
uint8_t presets_selectedBank() {
    return s_bank;
}

uint8_t presets_bankCount() {
    return FAVS;
}
